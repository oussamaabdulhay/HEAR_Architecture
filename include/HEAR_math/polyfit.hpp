...
template <typename T>
std::vector<T> polyfit(const std::vector<T> &xValues, const std::deque<T> &yValues, const int degree, const std::vector<T>& weights = std::vector<T>())
{
    using namespace boost::numeric::ublas;
...
template<typename T>
T polyval( const std::vector<T>& coefficients, const T& xVal ){
    size_t nDegree = coefficients.size();
    T yVal = 0;
    T xPowered = 1;
    for (size_t j = 0; j < nDegree; j++ )
...
template<typename T>
std::vector<T> polyder(const std::vector<T>& coefficients){
    size_t nCount = coefficients.size();
    std::vector<T> yValues( nCount-1 );
    for (size_t i = 1; i < nCount; i++ )
    {
        yValues[i-1] = i*coefficients[i];
    }
    return yValues;
}